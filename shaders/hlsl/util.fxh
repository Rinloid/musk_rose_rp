...
float4 texture2D_AA_lod(in Texture2D source, in sampler bilinearSampler, in float2 originalUV) {
	...
	const float2 adjustedUV = (adjustedFractionalTexel + floor(originalUV * TEXTURE_DIMENSIONS.xy)) / TEXTURE_DIMENSIONS.xy;
	const float4 blendedSample = source.SampleLevel(bilinearSampler, lerp(originalUV, adjustedUV, samplingMode), 0.0);
